...
var parser = {
	...
	"`((?!`)[\\S|\\s])+`": function(text) {
		return text.replace(new RegExp(this.match), function (item) { return '<b>' + item.substring(1,item.length-1) + '</b>'; });
	},
	"\\*((?!\\*)[\\S|\\s])+\\*": function(text) {
		return text.replace(new RegExp(this.match), function (item) { return '<i>' + item.substring(1,item.length-1) + '</i>'; });
	},
